...
function select_options($index, $itemId, $array, $placeholder)
{
    ...
    foreach ($array as $option) {
        if($option['id'] == $id)
            $options .= "<option selected value='". htmlspecialchars($id, ENT_QUOTES) ."'>". htmlspecialchars($option['name'], ENT_QUOTES) ."</option>";
        else
...
function is_unique_email($db, $emailAddress)
{
    ...
    return empty(
        database_query($db,"SELECT email FROM customers WHERE email = '{$emailAddress}'")
    );
}